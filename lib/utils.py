...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    ...
    train_dataset = torch.utils.data.TensorDataset(train_x_tensor, train_target_tensor,train_timestamp_tensor)
    ...
    val_dataset = torch.utils.data.TensorDataset(val_x_tensor, val_target_tensor,val_timestamp_tensor)
    ...
    test_dataset = torch.utils.data.TensorDataset(test_x_tensor, test_target_tensor,test_timestamp_tensor)
...
def compute_val_loss_mstgcn(net, val_loader, criterion,  masked_flag,missing_value,sw, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            encoder_inputs, labels , datetime_stamp = batch_data
            outputs = net(encoder_inputs,datetime_stamp)
            if masked_flag:
