...
class STEmbedding(nn.Module):
    ...
    def forward(self, SE, TE, T=288):
        ...
        SE = self.mlp_se(SE)  # [1,1,325,1] -> [1,1,325,64]
        # temporal embedding
        dayofweek = torch.empty(TE.shape[0], TE.shape[1], 7)  # [32,24,7]
        timeofday = torch.empty(TE.shape[0], TE.shape[1], T)  # [32,24,288] onehot
        for i in range(TE.shape[0]):
            dayofweek[i] = F.one_hot(TE[..., 0][i].to(torch.int64) % 7, 7)
        for j in range(TE.shape[0]):
            timeofday[j] = F.one_hot(TE[..., 1][j].to(torch.int64) % 288, T)
        TE = torch.cat((dayofweek, timeofday), dim=-1).to(device)  # [32,24,7+288=295]
        TE = TE.unsqueeze(dim=2)  # [32,24,1,7+288=295]
        TE = self.mlp_te(TE)  # [32,24,1,64]
        del dayofweek, timeofday
        return SE + TE  # [32,24,325,64]
